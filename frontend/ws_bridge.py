#!/usr/bin/env python3
"""A+B+C+D+E ZMQ -> WebSocket bridge for frontend/fullflow page."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="A+B+C+D+E 到 WebSocket 的实时桥接服务")
    ...
    parser.add_argument("--c-endpoint", default="tcp://localhost:5058", help="模块C订阅地址")
...
class ABWsBridge:
    ...
    def _compact_module_c_payload(self, payload: dict[str, Any], frame_id: int) -> dict[str, Any]:
        compact = {
            "frame_id": frame_id,
            "tracked_pedestrians": self._to_bool(payload.get("tracked_pedestrians")),
        }
        num_pedestrians = self._to_non_negative_int(payload.get("num_pedestrians"))
        if num_pedestrians is not None:
            compact["num_pedestrians"] = num_pedestrians
        return compact
    ...
    async def _zmq_loop(self) -> None:
        ...
        try:
            ...
            logging.info(
                "ZMQ订阅已启动: A=%s[%s], B=%s[%s], C=%s[%s], D=%s[%s], E=%s[%s]",
