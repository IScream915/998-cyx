...
用于同时订阅多个上游发布端，默认先按同一 `frame_id` 对齐 B+CD，再调用 `TrafficReminder` 处理。
当出现严重丢帧/持续不匹配时（`auto` 模式），会自动退化为“最新消息模式”，直接使用 B/C 最新消息继续运行。
...
- `frame_match`：只在同一 `frame_id` 的 B 和 CD 消息都到齐时输出结果
- `frame_match`：任一 `frame_id` 在配对超时内未到齐会被丢弃（保证低延迟）
- `latest`：忽略 `frame_id` 对齐，直接使用 B/C 最新消息处理（优先保证持续输出）
- `auto`（默认）：先 `frame_match`，当配对超时丢弃累计或连续 frame_id 不一致达到阈值时，自动切换到 `latest`
- 处理时会将 B+CD 数据映射为 `perception_json` 并调用 `TrafficReminder`
